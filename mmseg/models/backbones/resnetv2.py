...
from mmcv.cnn import MaxPool2d, build_activation_layer
...
class AvgPool2dSame(nn.AvgPool2d):
    ...
    def forward(self, x):
        return avg_pool2d_same(x, self.kernel_size, self.stride, self.padding,
                               self.ceil_mode, self.count_include_pad)


def max_pool2d_same(x,
                    kernel_size: List[int],
                    stride: List[int],
                    padding: List[int] = (0, 0),
...
class DownsampleAvg(nn.Module):
    ...
    def __init__(self,
        ...
        if stride > 1 or dilation > 1:
            avg_pool_fn = AvgPool2dSame if avg_stride == 1 and dilation > 1 else nn.AvgPool2d  # noqa
...
class StdConv2d(nn.Conv2d):
    ...
    Used for BiT ResNet-V2 models.
    Paper: `Micro-Batch Training with Batch-Channel
    ...
    def forward(self, x):
        x = F.conv2d(x, self.get_weight(), self.bias, self.stride,
...
class PreActBottleneck(nn.Module):
    ...
    Networks":     https://github.com/KaimingHe/resnet-1k-layers
    /blob/master/resnet-pre-act.lua Except it puts the stride on 3x3 conv when
    available.
    """
...
def create_resnetv2_stem(in_chs,
    ...
    if 'fixed' in stem_type:
        ...
        stem['pool'] = MaxPool2d(
            kernel_size=3, stride=2, padding=get_padding(3, 2))
    else:
...
class ResNetV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.stem = create_resnetv2_stem(
            ...
            conv_layer=conv_layer,
            norm_layer=norm_layer)
        stem_feat = ('stem.conv3' if 'deep' in stem_type else
        ...
        if isinstance(pretrained, str):
            ...
            with torch.no_grad():
                ...
                if self.stem.conv.weight.shape[1] == 1:
                    self.stem.conv.weight.copy_(
                        stem_conv_w.sum(dim=1, keepdim=True))
                ...
                if self.head.fc.weight.shape[0] == weights[
                        f'{prefix}head/conv2d/kernel'].shape[-1]:
                    self.head.fc.weight.copy_(
                        tf2th(weights[f'{prefix}head/conv2d/kernel']))
                    self.head.fc.bias.copy_(
                        tf2th(weights[f'{prefix}head/conv2d/bias']))
                for i, (sname,
                        stage) in enumerate(self.stages.named_children()):
                    for j, (bname,
                            block) in enumerate(stage.blocks.named_children()):
                        convname = 'standardized_conv2d'
                        block_prefix = f'{prefix}block{i + 1}/unit{j + 1:02d}/'
                        block.conv1.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}a/{convname}/kernel']))
                        block.conv2.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}b/{convname}/kernel']))
                        block.conv3.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}c/{convname}/kernel']))
                        block.norm1.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}a/group_norm/gamma']))
                        block.norm2.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}b/group_norm/gamma']))
                        block.norm3.weight.copy_(
                            tf2th(
                                weights[f'{block_prefix}c/group_norm/gamma']))
                        ...
                        if block.downsample is not None:
                            w = weights[
                                f'{block_prefix}a/proj/{convname}/kernel']
                            block.downsample.conv.weight.copy_(tf2th(w))
        elif pretrained is None:
            for n, m in self.named_modules():
                if isinstance(m, nn.Linear) or ('.fc' in n
